...
export default function AdminContactSettings() {
  ...
  const Field = ({ label, field, textarea, placeholder }) => (
    <div className="space-y-1.5">
      <Label className="text-xs text-slate-500">{label}</Label>
      {textarea ? (
        <Textarea value={form[field] || ""} onChange={(e) => set(field, e.target.value)} placeholder={placeholder} className="rounded-xl text-sm" rows={3} />
      ) : (
        <Input value={form[field] || ""} onChange={(e) => set(field, e.target.value)} placeholder={placeholder} className="rounded-xl text-sm" />
      )}
    </div>
  );
        ...
        {isLoading ? (
          ...
          <div className="space-y-6">
            ...
            <div className="bg-white rounded-2xl border border-slate-100 p-6 space-y-5">
              ...
              <Field label="Section Title" field="section_title" placeholder="Get in Touch" />
              <Field label="Email" field="email" placeholder="info@shoreplay.com" />
              <Field label="Phone" field="phone" placeholder="+86 123 456 7890" />
              <Field label="WhatsApp (with country code, no spaces or +)" field="whatsapp" placeholder="861234567890" />
              <Field label="Address" field="address" placeholder="City, Province, Country" />
              <Field label="Working Hours" field="working_hours" placeholder="Mon–Fri, 9:00 AM – 6:00 PM (GMT+8)" />
            </div>

            {/* Wholesale Card */}
            <div className="bg-white rounded-2xl border border-slate-100 p-6 space-y-5">
              <h2 className="text-sm font-semibold text-slate-700 border-b border-slate-100 pb-3">Wholesale / B2B Card</h2>
              <Field label="Card Title" field="wholesale_title" placeholder="For Wholesale Inquiries" />
              <Field label="Card Text" field="wholesale_text" placeholder="Description..." textarea />
              <Field label="Wholesale Email" field="wholesale_email" placeholder="wholesale@shoreplay.com" />
            </div>
